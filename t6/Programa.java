...
public class Programa extends Application {
    ...
    public void start(Stage stage){
        ...
        pane.setOnMouseDragged(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                try {
                    No n = g.buscaNo(event.getX(), event.getY());
                    n.retornaCirculo().setFill(Color.RED);
                    for (Aresta a : n.retornaListaDeArestas()) {
                        if (n.retornaCirculo().getCenterX() == a.retornaAresta().getStartX() && n.retornaCirculo().getCenterY() == a.retornaAresta().getStartY()) {
                            a.retornaAresta().setStartX(event.getX());
                            a.retornaAresta().setStartY(event.getY());
                        } else if (n.retornaCirculo().getCenterX() == a.retornaAresta().getEndX() && n.retornaCirculo().getCenterY() == a.retornaAresta().getEndY()) {
                            a.retornaAresta().setEndX(event.getX());
                            a.retornaAresta().setEndY(event.getY());
                        }
                    }
                    n.retornaCirculo().setCenterX(event.getX());
                    n.retornaCirculo().setCenterY(event.getY());
                    pane.setOnMouseReleased(new EventHandler<MouseEvent>() {
                        public void handle(MouseEvent event) {
                            for (No ref : g.retornaArrayNos()) {
                                ref.retornaCirculo().setFill(Color.BLUE);
                            }
                        }
                    });
